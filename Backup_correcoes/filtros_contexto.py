...
class FiltroAntiExaustao:
    ...
    def detectar_exaustao(df: pd.DataFrame, tipo_sinal: str, rsi: float) -> FiltroResult:
        ...
        # Bloquear se 3+ sinais de exaustão
        bloqueado = sinais_exaustao >= 3
        confianca = min(sinais_exaustao / 5.0, 1.0)
        
        motivo = f"Exaustão detectada: {', '.join(motivos)}" if bloqueado else ""
...
class FiltroSupporteResistenciaReal:
    """📊 FILTRO S/R REAL: Detecta zonas de S/R verdadeiras"""
    
    @staticmethod
    def analisar_sr_real(df: pd.DataFrame, tipo_sinal: str, lookback: int = 50) -> FiltroResult:
        """Analisa suporte/resistência real com múltiplos toques"""
        if len(df) < lookback:
            return FiltroResult(False, "", 0.0, {})
        ...
        # VERIFICAR SE ESTÁ PRÓXIMO DE NÍVEL FORTE
        bloqueado = False
        motivo = ""
        max_forca = 0
        nivel_conflitante = None
        
        # Para CALL, verificar resistências próximas
        if 'CALL' in tipo_sinal:
            for res in resistencias_fortes:
                if res['distancia'] < 0.005 and res['toques'] >= 3:  # Muito próximo e forte
                    if res['forca'] > max_forca:
                        max_forca = res['forca']
                        nivel_conflitante = res
                        bloqueado = True
                        motivo = f"Resistência forte: {res['nivel']:.5f} ({res['toques']} toques)"
        
        # Para PUT, verificar suportes próximos
        elif 'PUT' in tipo_sinal:
            for sup in suportes_fortes:
                if sup['distancia'] < 0.005 and sup['toques'] >= 3:  # Muito próximo e forte
                    if sup['forca'] > max_forca:
                        max_forca = sup['forca']
                        nivel_conflitante = sup
                        bloqueado = True
                        motivo = f"Suporte forte: {sup['nivel']:.5f} ({sup['toques']} toques)"
        
        confianca = min(max_forca / 50.0, 1.0) if max_forca > 0 else 0.0
        ...
        return FiltroResult(
            ...
            dados_extras={
                ...
                'nivel_conflitante': nivel_conflitante,
                'forca_maxima': max_forca,
...
class FiltroAntiLateral:
    ...
    def detectar_lateralizacao(df: pd.DataFrame, periodo: int = 20) -> FiltroResult:
        ...
        if range_pct < 0.15:  # Range muito pequeno
            sinais_lateral += 2
            motivos.append(f"Range pequeno: {range_pct:.3f}%")
        
        if cv < 0.8:  # Baixa volatilidade
            sinais_lateral += 1
        ...
        if movimento_medio_pct < 0.05:  # Movimentos muito pequenos
            sinais_lateral += 1
        ...
        # Bloquear se 3+ sinais de lateralização
        bloqueado = sinais_lateral >= 3
...
class FiltrosContextoMaster:
    """🛡️ MASTER CLASS: Coordena todos os filtros contextuais"""
    ...
    def analisar_contexto_completo(self, df: pd.DataFrame, analise_completa: Dict, 
        ...
        """Análise completa de contexto usando todos os filtros"""
        ...
        if self.config['usar_filtro_exaustao']:
            ...
            if resultado_exaustao.bloqueado:
        ...
        # Ajuste de score baseado na análise
        if not entrada_segura:
            # Penalizar score baseado na confiança dos filtros
            ajuste_score_total = -min(50, score_confianca * 100)
        else:
            # Pequeno bônus se passou por todos os filtros
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            'taxa_bloqueio_total': ((total - sum(self.stats[k] for k in self.stats if 'bloqueios' in k)) / total * 100),
    ...
    def gerar_relatorio_filtros(self) -> str:
        ...
        relatorio = f"""
🛡️ RELATÓRIO FILTROS CONTEXTO ANTI-LOSS
...
"""
        ...
        relatorio += "\n🛡️ FILTROS CONTEXTO PROTEGENDO CONTRA LOSSES!"
...
print("✅ FILTROS CONTEXTO ANTI-LOSS CARREGADOS - PROTECTION ACTIVE!")
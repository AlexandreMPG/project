...
class AIModelTrainer:
    ...
    def extrair_features_mercado(self, timestamp: int, par: str) -> Dict[str, float]:
        ...
        # Corrigir timestamp para extrair hora corretamente
        try:
            # Tentar diferentes formatos de timestamp
            if timestamp > 1e12:  # Timestamp em milissegundos
                timestamp_corrigido = timestamp / 1000
            else:
                timestamp_corrigido = timestamp
                
            # Verificar se o timestamp está em um range válido
            if timestamp_corrigido < 0 or timestamp_corrigido > time.time() + 86400:
                timestamp_corrigido = time.time()
                
            hour = datetime.datetime.fromtimestamp(timestamp_corrigido).hour
        except (ValueError, OSError, OverflowError):
            # Se ainda der erro, usar hora atual
    ...
    def criar_feature_vector(self, row: pd.Series) -> Optional[FeatureVector]:
        ...
        try:
            ...
            # Extrair hora com tratamento de erro
            try:
                ...
                hora = datetime.datetime.fromtimestamp(timestamp).hour
            except (ValueError, OSError, OverflowError):
